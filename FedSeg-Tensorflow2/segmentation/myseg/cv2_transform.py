...
def _active_numpy_rng():
    return getattr(_THREAD_LOCAL, "numpy_rng", None)


@contextmanager
def numpy_rng_scope(seed):
    previous_rng = _active_numpy_rng()
    _THREAD_LOCAL.numpy_rng = np.random.RandomState(int(seed))
    try:
        yield
    finally:
        if previous_rng is None:
            delattr(_THREAD_LOCAL, "numpy_rng")
        else:
            _THREAD_LOCAL.numpy_rng = previous_rng


def _rng_uniform(low, high):
    rng = _active_numpy_rng()
    if rng is None:
...
class RandomResizedCrop(object):
    ...
    def __call__(self, im_lb):
        ...
        scale = _rng_uniform(min(self.scales), max(self.scales))
