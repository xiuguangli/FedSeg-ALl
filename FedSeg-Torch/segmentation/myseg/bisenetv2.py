...
class SegmentHead(nn.Module):
    ...
    def __init__(self, args, in_chan, mid_chan, n_classes, up_factor=8, aux=True):
        ...
        self.drop = nn.Identity() if getattr(args, "debug_disable_dropout", False) else nn.Dropout(0.1)
        self.dropout_p = 0.1
    ...
    def _deterministic_dropout(self, feat):
        if (not self.training) or getattr(self.args, "debug_disable_dropout", False):
            return feat
        keep_prob = 1.0 - self.dropout_p
        n, c, h, w = feat.shape
        device = feat.device
        n_idx = torch.arange(n, device=device, dtype=torch.int64).view(n, 1, 1, 1)
        c_idx = torch.arange(c, device=device, dtype=torch.int64).view(1, c, 1, 1)
        h_idx = torch.arange(h, device=device, dtype=torch.int64).view(1, 1, h, 1)
        w_idx = torch.arange(w, device=device, dtype=torch.int64).view(1, 1, 1, w)
        flat_idx = (((n_idx * c + c_idx) * h + h_idx) * w + w_idx)
        seed = int(getattr(self.args, "seed", 1))
        hashed = torch.remainder(flat_idx * 1103515245 + 12345 + seed, 2147483647)
        mask = (hashed.to(torch.float32) / 2147483647.0) < keep_prob
