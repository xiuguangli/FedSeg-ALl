...
class CriterionPixelPairG(nn.Module):
    ...
    def forward(self, feat_S, feat_T,proto_mem,proto_mask):
        ...
        if self.args.kmean_num>0:
            ...
            proto_labels = torch.arange(C_, device=device)
            ...
            if cls_ != 255:
                ...
                if len(selected_index_)>num_p_c:
                    if getattr(self.args, "debug_deterministic_contrast", False):
                        selected_index_ = selected_index_[:num_p_c]
                    else:
...
#        exit()
        ...
        loss = loss.mean()
        if torch.isnan(loss):
            logger.error("NaN contrast loss")
            logger.error("logits={}", torch.unique(logits))
            logger.error("exp_logits={}", torch.unique(exp_logits))
            logger.error("neg_logits={}", torch.unique(neg_logits))
            logger.error("log_prob={}", torch.unique(log_prob))
            logger.error("mask_sum={}", torch.unique(mask.sum(1)))
            logger.error("mask={}", mask)
            logger.error("anchor_labels={}", torch.unique(anchor_labels))
            logger.error("proto_labels={}", proto_labels)
            logger.error("unique_proto_labels={}", torch.unique(proto_labels))
...
######################################
        ...
        loss = loss.mean()
        if torch.isnan(loss):
            logger.error("NaN local contrast loss")
            logger.error("logits={}", torch.unique(logits))
            logger.error("exp_logits={}", torch.unique(exp_logits))
            logger.error("neg_logits={}", torch.unique(neg_logits))
            logger.error("log_prob={}", torch.unique(log_prob))
