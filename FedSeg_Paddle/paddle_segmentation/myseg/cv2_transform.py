...
class TransformationTrain(object):
    ...
    def __init__(self, scales, cropsize):
        self.trans_func = paddle.vision.transforms.Compose(
            transforms=[RandomResizedCrop(scales, cropsize), RandomHorizontalFlip()]
        )
...
class RandomResizedCrop(object):
    ...
    def __call__(self, im_lb):
        if self.size is None:
            return im_lb
        im, lb = im_lb["im"], im_lb["lb"]
        assert tuple(im.shape)[:2] == tuple(lb.shape)[:2]
        crop_h, crop_w = self.size
        scale = np.random.uniform(min(self.scales), max(self.scales))
        im_h, im_w = [math.ceil(el * scale) for el in tuple(im.shape)[:2]]
...
class ToTensor(object):
    ...
    def __call__(self, im_lb):
        ...
        im = paddle.to_tensor(data=im).divide_(y=paddle.to_tensor(255))
        dtype, device = im.dtype, im.place
        mean = paddle.to_tensor(data=self.mean, dtype=dtype, place=device)[
            :, None, None
        ]
        std = paddle.to_tensor(data=self.std, dtype=dtype, place=device)[:, None, None]
        im = (
            im.subtract_(y=paddle.to_tensor(mean))
            .divide_(y=paddle.to_tensor(std))
            .clone()
        )
        if not lb is None:
            lb = paddle.to_tensor(data=lb.astype(np.int64).copy()).clone()
        return dict(im=im, lb=lb)
